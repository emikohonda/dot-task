...
export class SchedulesService {
  ...
  async create(input: {
    ...
    if (input.startTime && !isValidHm(input.startTime)) {
      throw new BadRequestException('startTime must be HH:mm');
    }
    if (input.endTime && !isValidHm(input.endTime)) {
  ...
  async update(id: string, input: {
    ...
    const nextStartTime =
      input.startTime !== undefined ? input.startTime : exists.startTime;
    const nextEndTime =
      input.endTime !== undefined ? input.endTime : exists.endTime;

    if (nextStartTime && !isValidHm(nextStartTime)) {
      throw new BadRequestException('startTime must be HH:mm');
    }
    if (nextEndTime && !isValidHm(nextEndTime)) {
      throw new BadRequestException('endTime must be HH:mm');
    }
    ...
    const contractorIds =
      input.contractorIds !== undefined ? uniq(input.contractorIds).filter(Boolean) : undefined;
    const employeeIds =
      input.employeeIds !== undefined ? uniq(input.employeeIds).filter(Boolean) : undefined;
