...
async function fetchContractors(params: URLSearchParams): Promise<PaginatedContractors> {
  try {
    const res = await fetch(`${API_BASE}/contractors?${params.toString()}`, { cache: "no-store" });
    if (!res.ok) return { items: [], total: 0, limit: PAGE_LIMIT, offset: 0 };
    const data = await res.json();
    if (data && Array.isArray(data.items)) return data as PaginatedContractors;
...
function ContractorsPageInner() {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    fetchContractors(params).then((data) => {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        title="外注先一覧"
        right={
          <Link href="/contractors/new" className="rounded-full bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-sky-700">
            ＋ 外注先を追加
          </Link>
        }
      />
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center">
            <p className="text-sm font-medium text-slate-900">
              {hasFilter ? "条件に一致する外注先はありません" : "まだ外注先が登録されていません"}
            </p>
            <p className="mt-1 text-sm text-slate-600">
              {hasFilter ? "絞り込み条件を変えてみてください。" : "右上の「＋ 外注先を追加」から登録できます。"}
            </p>
          </div>
        ) : (
          <>
                  ...
                  {contractors.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-100/60 [&>td]:border-b [&>td]:border-dashed [&>td]:border-slate-200 last:[&>td]:border-b-0">
                      ...
                      <td className="w-[88px] py-4 text-center">
                        <Link href={`/contractors/${item.id}`} className="inline-flex h-9 items-center justify-center rounded-xl border border-slate-200 bg-white px-3 text-xs font-medium text-slate-900 hover:bg-slate-50">
                          開く
                        </Link>
                      </td>
                    </tr>
